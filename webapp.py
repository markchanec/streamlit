...
if buttonID:
    randomID = dbBroker.getRandomID()
    tokenID = slot.number_input("Enter a number", 1, 90000, randomID[0], key="ID") #max to be limited as per csv for production copy
else:
    tokenID = slot.number_input("Enter a number", 1, 90000, INIT_ID, key="ID") #max to be limited as per csv for production copy
...
print("ID =", tokenID)
Iqr, Kde = dbBroker.init_webapp(tokenID)
